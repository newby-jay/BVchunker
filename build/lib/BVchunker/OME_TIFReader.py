...
class ReadFromOME_TIFVid(PTransform):
    """A PTransform for reading OME TIF video files."""

    def __init__(self, pattern, chunkShape=None, Overlap=None, downSample=1):
        """Initializes ``ReadFromOME_TIFVid``."""
        super(ReadFromOME_TIFVid, self).__init__()
        self._source = _OME_TIFSource(pattern)
        self.chunkShape = chunkShape
        self.Overlap = Overlap
        self.downSample = downSample
...
class _OME_TIFutils:
    ...
    def readHeader(self, vfile):
        ...
        imgMD = self.read_metadata(imgMD)
        vfile.seek(0)
        return A0, nextIFD, imgOffset, imgMD

class _OME_TIFSource(filebasedsource.FileBasedSource):
    """Read OME TIF video into chunks."""

    def __init__(self, file_pattern):
        super(_OME_TIFSource, self).__init__(
            file_pattern,
            splittable=True,
            min_bundle_size=0,
            validate=False)
            ...
            try:
                ...
                while range_tracker.try_claim(vfile.tell()):
                    ...
                    if 0 < nextIFD <= fileSize - markerSize:
                        ...
                        IFD, nextIFD, imgOffset, imgMD = TU.getNextOffset(vfile) # leave vfile at end of IFD
                        next_block_start = nextIFD + markerSize
                        assert nextIFD - IFD == blockSize or nextIFD == fileSize
                        assert imgMD['Nx'] == Nx
                        assert imgMD['Ny'] == Ny
